...
`data/nikkei-contribution.json`は、計算処理、JSON形式、失敗時保護を確認するための試験データです。実際の株価や公式の日経平均寄与度ではありません。

JSONの`is_trial: true`は、次の状態を表します。

- 架空の終値、仮の株価換算係数、仮の除数で生成している
- 主要25銘柄だけを対象とした部分的な処理試験である
- 正確性や最新性を保証せず、投資判断には利用できない
- 本番表示用データとして扱わず、フロントエンドから読み込まない

現時点では`nikkei-contribution.html`などの一般公開ページを作成せず、既存のマーケットボード、ブログ、ナビゲーションからも試験JSONへ接続しません。

本番表示への切り替えは、次のすべてについて公開サイトでの利用、加工、派生値表示を認めるデータ契約または権利者の許諾を確保した場合に限ります。
...
条件を満たした後、225銘柄の完全性、日付、銘柄入れ替え、株式分割・併合、PAFと除数の変更を検証できる運用を整備し、試験JSONとは別のレビューを経て本番表示へ接続します。
...
- 現在のJSONは`is_trial: true`の試験データであり、一般公開ページやフロントエンドには接続しません。
